...
export const LEADS_PAGE_SIZE_OPTIONS = [10, 25, 50, 100] as const;
export type LeadsPageSize = (typeof LEADS_PAGE_SIZE_OPTIONS)[number];

export const LEADS_PAGE_SIZE_STORAGE_KEY = 'loro:leads-page-size';

export function readStoredLeadsPageSize(): LeadsPageSize {
  if (typeof window === 'undefined') return 25;
  try {
    const raw = localStorage.getItem(LEADS_PAGE_SIZE_STORAGE_KEY);
    const n = raw != null ? Number(raw) : NaN;
    if (LEADS_PAGE_SIZE_OPTIONS.includes(n as LeadsPageSize)) return n as LeadsPageSize;
  } catch {
    /* ignore */
  }
  return 25;
...
export interface LeadsListPaginationProps {
  page: number;
  totalPages: number;
  total: number;
  ...
  onPageSizeChange: (size: LeadsPageSize) => void;
  className?: string;
}
...
export function LeadsListPagination({
  page,
  totalPages,
  total,
  pageSize,
    ...
    >
      <div className="flex items-center gap-2 text-xs text-muted-foreground sm:text-sm">
        ...
        <Select
          ...
          onValueChange={(v) => {
            const n = Number(v);
            if (LEADS_PAGE_SIZE_OPTIONS.includes(n as LeadsPageSize)) {
              onPageSizeChange(n as LeadsPageSize);
            }
          }}
        >

...
import { CLIENTS_QUERY_KEY_PREFIX } from '@/api/hooks/use-clients';
...
export function useVisualiserPrefetch(options: {
  ...
  useEffect(() => {
    ...
    void queryClient.prefetchInfiniteQuery({
      queryKey: [...CLIENTS_QUERY_KEY_PREFIX, 'infinite', '', '', ''],
      queryFn: async ({ pageParam }) =>
        getClients(client, { page: pageParam as number, limit: 100 }),
      initialPageParam: 1,
      staleTime: 60 * 1000,
    });

...
export const PAYSLIPS_PAGE_SIZE_OPTIONS = [10, 25, 50, 100] as const;
export type PayslipsPageSize = (typeof PAYSLIPS_PAGE_SIZE_OPTIONS)[number];

export const PAYSLIPS_PAGE_SIZE_STORAGE_KEY = 'loro:payslips-page-size';

export function readStoredPayslipsPageSize(): PayslipsPageSize {
  if (typeof window === 'undefined') return 25;
  try {
    const raw = localStorage.getItem(PAYSLIPS_PAGE_SIZE_STORAGE_KEY);
    const n = raw != null ? Number(raw) : NaN;
    if (PAYSLIPS_PAGE_SIZE_OPTIONS.includes(n as PayslipsPageSize)) {
      return n as PayslipsPageSize;
    }
  } catch {
    /* ignore */
  }
...
export interface PayslipsListPaginationProps {
  page: number;
  totalPages: number;
  total: number;
  ...
  onPageSizeChange: (size: PayslipsPageSize) => void;
  className?: string;
}
...
export function PayslipsListPagination({
  page,
  totalPages,
  total,
  pageSize,
      ...
      <div className="flex items-center gap-2 text-xs text-muted-foreground sm:text-sm">
        ...
        <Select
          ...
          onValueChange={(v) => {
            const n = Number(v);
            if (PAYSLIPS_PAGE_SIZE_OPTIONS.includes(n as PayslipsPageSize)) {
              onPageSizeChange(n as PayslipsPageSize);
            }
          }}
        >
          ...
          <SelectContent>
            {PAYSLIPS_PAGE_SIZE_OPTIONS.map((size) => (

...
export type ReportsPageSize = (typeof REPORTS_PAGE_SIZE_OPTIONS)[number];

export const REPORTS_PAGE_SIZE_STORAGE_KEY = 'loro:reports-targets-page-size';

export function readStoredReportsPageSize(): ReportsPageSize {
  if (typeof window === 'undefined') return 25;
  try {
    const raw = localStorage.getItem(REPORTS_PAGE_SIZE_STORAGE_KEY);
    const n = raw != null ? Number(raw) : NaN;
    if (REPORTS_PAGE_SIZE_OPTIONS.includes(n as ReportsPageSize)) {
      return n as ReportsPageSize;
    }
  } catch {
    /* ignore */
  }
...
export interface ReportsListPaginationProps {
  page: number;
  totalPages: number;
  total: number;
...
export function ReportsListPagination({
  ...
  return (
    <div
      ...
      data-tour="reports-targets-pagination"
    >
      <div className="flex items-center gap-2 text-xs text-muted-foreground sm:text-sm">
        ...
        <Select
          ...
          onValueChange={(v) => {
            const n = Number(v);
            if (REPORTS_PAGE_SIZE_OPTIONS.includes(n as ReportsPageSize)) {
              onPageSizeChange(n as ReportsPageSize);
            }
          }}
        >
